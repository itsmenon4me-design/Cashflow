// Sustained typing after empty results: height must stay constant
...
(async () => {
  ...
  const sample = () => page.evaluate(() => {
    ...
    return { y: +r.y.toFixed(1), h: +r.height.toFixed(1), skeleton: isSkeleton };
  });
  ...
  for (const path of ['/transactions', '/incomes', '/expenses']) {
    ...
    await input.click();
    // first search -> empty
    await input.type('zz', { delay: 60 });
    await page.waitForTimeout(1500);
    // now keep typing (each char = new search while empty) and sample rapidly
    const heights = new Set();
    let skeletonSeen = false;
    for (const ch of ['z', 'z', 'z', 'z']) {
      await input.press(ch);
      for (let i = 0; i < 8; i++) {
        const s = await sample();
        if (s) {
          heights.add(s.h);
          if (s.skeleton) skeletonSeen = true;
        }
        await page.waitForTimeout(60);
      }
    }
    const stable = heights.size === 1 && !skeletonSeen;
    console.log(`${path}: heights=[${[...heights].join(', ')}] skeletonFlash=${skeletonSeen} => ${stable ? 'STABIL' : '<<< BERGERAK'}`);
    await page.screenshot({ path: `./playwright-traces/stable-empty-${path.slice(1)}.png` });
  }
  await browser.close();
})();
