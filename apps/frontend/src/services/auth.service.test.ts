...
describe("auth.service", () => {
  ...
  it("requests the GitHub OAuth redirect URL", async () => {
    mockedApi.get.mockResolvedValue({
      success: true,
      url: "https://github.com/login/oauth/authorize",
    });

    const res = await authService.githubLogin();

    expect(apiClient.get).toHaveBeenCalledWith("/auth/github");
    expect(res).toEqual({
      success: true,
      url: "https://github.com/login/oauth/authorize",
    });
  ...
  it("register posts to /auth/register and returns payload", async () => {
    ...
    const returned = {
      success: true,
      data: { id: "u1", email: payload.email },
    };
  ...
  it("forgotPassword posts to /auth/email/forgot-password", async () => {
    ...
    expect(apiClient.post).toHaveBeenCalledWith("/auth/email/forgot-password", {
      email: "user@example.com",
    });
