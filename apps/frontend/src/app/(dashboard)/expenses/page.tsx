...
export default function Page() {
  ...
  // Optimistic UI: offline-created expense transactions appear immediately with
  ...
  const visibleTransactions = useMemo(() => {
    ...
    const pendingExpense = pendingItems.filter((txn) => txn.type === "expense");
    return [...pendingExpense, ...flagged];
  }, [transactions, pendingItems, pendingIds]);

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-semibold tracking-tight text-foreground">{uiText.navigation.expense}</h1>
