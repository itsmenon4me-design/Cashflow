...
describe('AnalyticsService', () => {
  it.each(['IDR', 'USD', 'SGD', 'EUR'])(
    'returns a zero score for empty financial data in %s',
    async (currency) => {
      const monthly = {
      ...
      const prisma = { account: { aggregate: jest.fn().mockResolvedValue({ _sum: { current_balance_cents: 0 } }) } } as any;
  ...
  it('keeps the normal formula for non-empty data', async () => {
    ...
    const prisma = { account: { aggregate: jest.fn().mockResolvedValue({ _sum: { current_balance_cents: 100000 } }) } } as any;
