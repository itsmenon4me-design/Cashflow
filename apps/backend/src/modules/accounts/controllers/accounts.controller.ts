...
export class AccountsController {
  ...
  @ApiBearerAuth('jwt')
  @UseGuards(JwtAuthGuard)
  async create(@Req() req: Request & { user?: { sub?: string } }, @Body() body: CreateAccountDto) {
    const userId = req.user?.sub as string;
    const created = await this.accounts.create(userId, body as any);
  ...
  @ApiBearerAuth('jwt')
  @UseGuards(JwtAuthGuard)
  async update(@Req() req: Request & { user?: { sub?: string } }, @Param('id') id: string, @Body() body: UpdateAccountDto) {
    const userId = req.user?.sub as string;
    const updated = await this.accounts.update(userId, id, body as any);
  ...
  @ApiBearerAuth('jwt')
  @UseGuards(JwtAuthGuard)
  async setDefault(@Req() req: Request & { user?: { sub?: string } }, @Param('id') id: string) {
    const userId = req.user?.sub as string;
