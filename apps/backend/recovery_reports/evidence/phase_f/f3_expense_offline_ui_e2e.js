...
(async ()=>{
  ...
  const jsonOut = path.join(outDir, `f3_expense_offline_e2e_${timestamp}.json`);
  const mdOut = path.join(outDir, `f3_expense_offline_e2e_${timestamp}.md`);
  const screenshotOut = path.join(outDir, `f3_expense_offline_e2e_${timestamp}.png`);
  ...
  try{
    ...
    if(!accessToken){ fs.writeFileSync(path.join(outDir, `f3_expense_offline_login_failed_${timestamp}.html`), await page.content()); throw new Error('Login did not produce access token'); }
    ...
    const uniqueRef = `offline-e2e-expense-${Date.now()}-${Math.random().toString(36).slice(2,8)}`;
    const payload = {
      ...
      note: `Offline Expense ${uniqueRef}`,
    ...
    const md = [`# F.3 Expense Offline UI E2E - ${timestamp}`, '', `UniqueRef: ${uniqueRef}`, '', `enqueueResult: ${JSON.stringify(enqueueOk)}`, '', `flushResult: ${JSON.stringify(flushResult)}`, '', `dbCount: ${dbCount}`].join('\n');
    fs.writeFileSync(mdOut, md);

    console.log('Expense offline E2E complete. Results written to', jsonOut, mdOut, screenshotOut);
    await browser.close();
    process.exit(0);
  }catch(e){
    try{ fs.writeFileSync(path.join(outDir, `f3_expense_offline_error_${timestamp}.log`), String(e.stack||e)); }catch(e){}
    console.error('Expense offline e2e error', e);
