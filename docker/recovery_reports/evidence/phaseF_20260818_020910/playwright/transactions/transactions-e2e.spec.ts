...
async function seedDb() {
  ...
  try {
    ...
    await client.query('DELETE FROM accounts WHERE id = $1', [ACCOUNT_ID]);
    await client.query('DELETE FROM categories WHERE id = $1', [CATEGORY_ID]);
    await client.query('DELETE FROM users WHERE id = $1 OR email = $2', [USER_ID, 'e2e@test.local']);
    ...
    await client.query(
      ...
      [USER_ID, 'e2e@test.local', 'e2euser', 'E2E User', 'e2etesthash'],
    );
    ...
    await client.query(
      ...
      [ACCOUNT_ID, USER_ID, 'E2E Account', 'CASH', 'IDR'],
    );
...
test.describe('Transactions page month-scoped filtering (E2E)', () => {
  ...
  test('initial render applies current-month fromDate/toDate and backend returns only current-month transactions', async ({ page }) => {
    ...
    // Prepare UI auth to skip actual login flow (local dev only)
    ...
    await page.evaluate(() => {
      try {
        ...
        localStorage.setItem('cashflow.user', JSON.stringify({ id: '00000000-0000-0000-0000-000000000001', name: 'E2E User', email: 'e2e@test.local' }));
    ...
    // Start capturing the /transactions network response (only XHR/fetch/json responses, not the page navigation)
    const transactionsResponse = page.waitForResponse((res) => {
      try {
        const url = res.url();
        if (!url.includes('/transactions')) return false;
        const r = res.request();
    ...
    // Wait for the /transactions response and inspect URL and body
